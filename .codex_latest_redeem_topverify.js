...
function playConfetti(durationMs = 4200) {
    ...
    function addPiece(originX, originY, direction) {
        ...
        let speedMin = 4;
        let speedRange = 4.8;
        let spread = (Math.random() - 0.5) * 0.9;
        let baseAngle = direction === 'left' ? -0.95 : -2.2;

        if (direction === 'top-left') {
            baseAngle = 0.72;
            spread = (Math.random() - 0.5) * 1.25;
            speedMin = 4.6;
            speedRange = 5.2;
    ...
    function burst(side, count) {
        const originX = side === 'left' ? width * 0.11 : width * 0.89;
        const originY = height * (0.72 + Math.random() * 0.12);
        const direction = side === 'left' ? 'left' : 'right';
        for (let i = 0; i < count; i++) {
            addPiece(originX, originY, direction);
        }
    }

    function burstTop(side, count) {
        const originX = side === 'left'
            ? width * (0.02 + Math.random() * 0.035)
            : width * (0.98 - Math.random() * 0.035);
        const originY = height * (0.02 + Math.random() * 0.05);
        const direction = side === 'left' ? 'top-left' : 'top-right';
        for (let i = 0; i < count; i++) {
            addPiece(originX, originY, direction);
        }
    }
    ...
    burstTop('left', 88);
    burstTop('right', 88);
    window.setTimeout(() => burst('left', 32), 180);
    window.setTimeout(() => burst('right', 32), 220);
    window.setTimeout(() => burstTop('left', 64), 140);
    window.setTimeout(() => burstTop('right', 64), 170);
    window.setTimeout(() => burstTop('left', 56), 320);
    window.setTimeout(() => burstTop('right', 56), 350);
    burstTimer = window.setInterval(() => {
        burst('left', 20 + Math.floor(Math.random() * 10));
        burst('right', 20 + Math.floor(Math.random() * 10));
        burstTop('left', 22 + Math.floor(Math.random() * 10));
        burstTop('right', 22 + Math.floor(Math.random() * 10));
    }, 540);
    animationId = window.requestAnimationFrame(tick);
}
