...
// 礼花动画（保留原本成功庆祝体验）
function playConfetti(durationMs = 4200) {
    ...
    const pieces = [];
    const palette = [4, 12, 22, 34, 46, 356];
    ...
    const directionConfig = {
        'top-left': { baseAngle: 0.74, spread: 1.25, speedMin: 4.8, speedRange: 5.3 },
        top: { baseAngle: Math.PI / 2, spread: 1.2, speedMin: 4.6, speedRange: 5.2 },
        'top-right': { baseAngle: Math.PI - 0.74, spread: 1.25, speedMin: 4.8, speedRange: 5.3 },
        right: { baseAngle: Math.PI, spread: 1.1, speedMin: 4.4, speedRange: 5.0 },
        'bottom-right': { baseAngle: -(Math.PI - 0.74), spread: 1.25, speedMin: 4.8, speedRange: 5.3 },
        bottom: { baseAngle: -Math.PI / 2, spread: 1.2, speedMin: 4.6, speedRange: 5.2 },
        'bottom-left': { baseAngle: -0.74, spread: 1.25, speedMin: 4.8, speedRange: 5.3 },
        left: { baseAngle: 0, spread: 1.1, speedMin: 4.4, speedRange: 5.0 }
    };

    const allSources = ['top-left', 'top', 'top-right', 'right', 'bottom-right', 'bottom', 'bottom-left', 'left'];

    function addPiece(originX, originY, direction) {
        const hue = palette[Math.floor(Math.random() * palette.length)];
        const config = directionConfig[direction] || directionConfig.left;
        const speed = config.speedMin + Math.random() * config.speedRange;
        ...
        pieces.push({
            ...
            color: `hsl(${hue}, 92%, ${52 + Math.random() * 14}%)`
        });
    }
    ...
    function getSourceOrigin(source) {
        switch (source) {
            case 'top-left':
                return { x: width * (0.02 + Math.random() * 0.05), y: height * (0.015 + Math.random() * 0.05) };
            case 'top':
                return { x: width * (0.2 + Math.random() * 0.6), y: height * (0.005 + Math.random() * 0.03) };
            case 'top-right':
                return { x: width * (0.93 + Math.random() * 0.05), y: height * (0.015 + Math.random() * 0.05) };
            case 'right':
                return { x: width * (0.975 + Math.random() * 0.01), y: height * (0.2 + Math.random() * 0.6) };
            case 'bottom-right':
                return { x: width * (0.93 + Math.random() * 0.05), y: height * (0.945 + Math.random() * 0.04) };
            case 'bottom':
                return { x: width * (0.2 + Math.random() * 0.6), y: height * (0.965 + Math.random() * 0.02) };
            case 'bottom-left':
                return { x: width * (0.02 + Math.random() * 0.05), y: height * (0.945 + Math.random() * 0.04) };
            case 'left':
            default:
                return { x: width * (0.005 + Math.random() * 0.02), y: height * (0.2 + Math.random() * 0.6) };
        }
    }

    function burstFrom(source, count) {
        const origin = getSourceOrigin(source);
        for (let i = 0; i < count; i++) {
            addPiece(origin.x, origin.y, source);
        }
    }

    function burstAll(cornerCount, edgeCount) {
        allSources.forEach(source => {
            const isCorner = source.includes('-');
            burstFrom(source, isCorner ? cornerCount : edgeCount);
        });
    }
    ...
    function draw() {
        ...
        for (let i = pieces.length - 1; i >= 0; i--) {
            ...
            ctx.fillRect(-p.size / 2, -p.size * 0.25, p.size, p.size * 0.55);
            ctx.restore();
        }
    }
    ...
    function tick(now) {
        ...
        if (now < endTime || pieces.length > 0) {
    ...
    cleanupConfetti = destroy;
    resizeCanvas();
    window.addEventListener('resize', resizeCanvas);

    burstAll(72, 54);
    window.setTimeout(() => burstAll(46, 36), 150);
    window.setTimeout(() => burstAll(40, 30), 300);
    burstTimer = window.setInterval(() => {
        burstAll(14 + Math.floor(Math.random() * 8), 10 + Math.floor(Math.random() * 6));
    }, 520);
    animationId = window.requestAnimationFrame(tick);
}
...
function playFireworks(durationMs = 4200) {
    ...
    function draw() {
        // Keep canvas transparent so success card stays visible under fireworks.
        ctx.clearRect(0, 0, width, height);
