...
async def create_default_settings():
    ...
    async with AsyncSessionLocal() as session:
        ...
        default_settings = [
            ...
            Setting(
                key="user_daily_signin_points",
                value=str(settings.user_daily_signin_points),
                description="每日签到奖励积分"
            ),
            Setting(
                key="shop_redeem_code_cost",
                value=str(settings.shop_redeem_code_cost),
                description="积分商城兑换码价格"
            ),
        ]
